  ...
  #include "../sd/cardreader.h"
    ...
    const char * MarlinUI::scrolled_filename(CardReader &theCard, const uint8_t maxlen, uint8_t hash, const bool doScroll) {
      ...
      if (theCard.longFilename[0]) {
        ...
        #else
          static char longFilenameCyr[LONG_FILENAME_LENGTH*3];
          theCard.longFilename[maxlen] = '\0'; // cutoff at screen edge
            ...
            memcpy(longFilenameCyr, theCard.longFilename, LONG_FILENAME_LENGTH);
            int n = 0;
            for(int i = 0; i < LONG_FILENAME_LENGTH && n < 2*LONG_FILENAME_LENGTH; i++) {
              char c1251 = theCard.longFilename[i];
              if(c1251 >= 0 && c1251 <= 0x7f) longFilenameCyr[n++] = c1251; else {
                int v = table[(int)(0x7f & c1251)];
                longFilenameCyr[n++] = (char)v;
                longFilenameCyr[n++] = (char)(v >> 8);
                if (v >>= 16)
                    longFilenameCyr[n++] = (char)v;
              }
              if(c1251 == 0) break;
            }
            longFilenameCyr[2*LONG_FILENAME_LENGTH - 1] = '\0';
            outstr = longFilenameCyr;
          #endif
        #endif
      }
      return outstr;
